...
void Sapling::calculateGrowth()
{
    ...
    if (!dead_wood.isEmpty() || !dead_fine.isEmpty())
        if (mRUS->ru()->snag())
        {
            double woody_aboveground_c = dead_wood.C;
            double fine_aboveground_c = dead_fine.C;
            mRUS->ru()->snag()->addToSoil(species, dead_wood, dead_fine, woody_aboveground_c, fine_aboveground_c);
        }
